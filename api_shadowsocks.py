...
class ShadowSocks(object):
    ...
    def setShadowSocks(self, pattern):
        ...
        if not pattern:
            ss_ja = re.findall(JapanA_pattern, self.content)
            ss_jb = re.findall(JapanB_pattern, self.content)
            ss_jc = re.findall(JapanC_pattern, self.content)
            ss_sa = re.findall(SingaporeA_pattern, self.content)
            ss_sb = re.findall(SingaporeB_pattern, self.content)
            ss_sc = re.findall(SingaporeC_pattern, self.content)

            items = ss_ja + ss_jb + ss_jc + ss_sa + ss_sb + ss_sc
        ...
        with open(self.ssConfigPath, "r+", encoding='utf-8') as f:
            ...
            data['strategy'] = 'com.shadowsocks.strategy.ha'
        ...
        try:
            subprocess.call('taskkill /f /im shadowsocks.exe',
...
# 基本变量
##################################################
url = 'https://us.ishadowx.net/'           # 所要爬取服务器密码的网站
headers = {
